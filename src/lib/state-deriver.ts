...
export function deriveState(evidence: AgentEvidence, now: number): DerivedState {
  if (!evidence.connectionHealthy) {
    return buildState("idle", 1, "Connection health is false; agent is treated as idle.");
  }
  ...
  if (isSleepHours(now) && freshestActivityAge > 15 * 60_000) {
    return buildState("idle", 0.6, "No recent activity during local sleep hours (1–6 AM). Treated as idle.");
  }

  if (freshestActivityAge > VERY_STALE_MS && (hasAnyWsEvidence || hasAnyHttpEvidence)) {
    return buildState("idle", 0.2, `All evidence is older than 5 minutes; freshest signal was ${formatAge(freshestActivityAge)}. Treated as idle.`);
  }

  if (!hasAnyWsEvidence && !hasAnyHttpEvidence) {
    return buildState("idle", 0, "No evidence available for this agent yet. Treated as idle.");
  }

  return buildState("idle", 0.1, "Evidence exists, but it is insufficient to derive a trustworthy state. Treated as idle.");
}
